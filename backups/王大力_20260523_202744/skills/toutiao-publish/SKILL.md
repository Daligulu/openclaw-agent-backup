...
version: 6.2.0
description: 自动发布内容到今日头条（微头条/文章）。触发词：发头条、发布头条、微头条、今日头条、发文章、写头条、存草稿。支持 AI 推荐图片插入正文、免费正版图片库封面、完整文章自动化发布、微头条存草稿。
...
## 🆕 2026-05-07 v6.2 新增：微头条存草稿

### 功能说明
通过 CDP（Chrome DevTools Protocol）协议直接操作 Chromium 浏览器，向微头条编辑器注入内容并保存为草稿。无需 OpenClaw Browser 工具，绕过浏览器启动超时问题。

### 实测结果（2026-05-07）
- **存草稿状态**: ✅ 成功
- **测试内容**: 26 字微头条
- **Toast 确认**: "保存成功"
- **方案**: CDP 协议 + ProseMirror 内容注入 + 按钮点击

### 快速使用
```bash
# 1. 确保 Chromium 已启动（带 --remote-allow-origins=*）
# 2. 执行存草稿
./save-draft.sh "你的微头条内容"

# 或从文件读取
./save-draft.sh --file content.txt
...
### 技术方案
1. **连接浏览器**: CDP WebSocket (`ws://127.0.0.1:9222`)
2. **导航到发布页**: `https://mp.toutiao.com/profile_v4/weitoutiao/publish`
3. **注入内容**: `.ProseMirror` innerHTML + 事件触发序列
4. **点击存草稿**: `button` text="存草稿" 匹配
5. **验证结果**: 检测 "保存成功" toast

### 关键选择器
| 元素 | 选择器 | 说明 |
|------|--------|------|
| 编辑器 | `.ProseMirror` | contenteditable div |
| 存草稿按钮 | `button` text="存草稿" | class: byte-btn-default |
| 发布按钮 | `button` text="发布" | class: byte-btn-primary |
