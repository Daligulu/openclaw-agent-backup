...
**降级**：image_gen.py 支持多 provider **并行竞速**（按 config.yaml 中 providers 列表同时启动，第一个完成的胜出，其他自动取消）。config.yaml 中可选 `timeout` 字段控制整体超时（默认 300 秒）。全部失败 → 输出提示词 + 备选图库关键词，继续。
...
**⚠️ 多账号发布**：publish 命令**不支持** `--account` 参数，默认只使用 config.yaml 顶部的 appid/secret。切换账号需临时修改配置：
...
python3 {skill_dir}/toolkit/cli.py publish ...
mv config.yaml.bak config.yaml
```

账号速查（config.yaml accounts 字段）：
- 峰AI路：`wx42b46ea46863a720` / `5bd9348a8a4dd8c23cf0059036461ea2`
- 狗狗生活小百科：`wx27855f8407f2c81c` / `01b109d8aa7d88c0c86aa28bc2f008ff`
...
| 生图失败 | 输出提示词 |
| 推送失败 | 本地 HTML |
| 微信 API 40007 错误 | 检查公众号权限，使用永久素材 media_id（非临时素材） |
| 历史写入 | 警告不阻断 |
| 效果数据 | 告知等 24h |
